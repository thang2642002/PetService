...
import {
  ...
  deletePetReview,
  updatePetReview,
} from "../../../../services/petReviewServices";
...
const Comment = ({ petId }) => {
  ...
  const fetchComments = async () => {
    try {
      const response = await getPetReviewById(petId);
  ...
  const handleDeleteComment = async (commentId) => {
    const data = await deletePetReview(commentId);
  ...
  const handleEditComment = (comment) => {
    setEditingCommentId(comment.pet_review_id);
  ...
  const handleUpdateComment = async (commentId) => {
    try {
      const data = await updatePetReview(
        commentId,
        rating,
        editedComment,
    ...
    <div className="container mx-auto my-6 p-4 bg-white shadow-md rounded-md">
      ...
      <div className="space-y-4">
        {comments.map((comment) => (
          <div
            key={comment.pet_review_id || comment.user_id}
          ...
          >
            ...
            <div className="flex-1">
              ...
              <div className="flex justify-between items-center text-gray-500">
                {editingCommentId === comment.pet_review_id ? (
                  <div className="flex flex-col">
                    ...
                    <button
                      ...
                      onClick={() => handleUpdateComment(comment.pet_review_id)}
                    >
                      Cập nhật
                    </button>
                  </div>
                ...
                {user?.user?.data?.user_id === comment?.user?.user_id && (
                  <div className="flex space-x-4 text-[13px] text-blue-500">
                    ...
                    <div
                      className="cursor-pointer"
                      onClick={() => handleDeleteComment(comment.pet_review_id)}
                    >
                      Xóa
                    </div>
                  </div>
